...
def map_nunchuck_joystick(n):
  # diagnostics.watch(wiimote[n].nunchuck.stick.x)
  # diagnostics.watch(wiimote[n].nunchuck.stick.y)

  # Right Stick
  vJoy[n].rx = filters.mapRange(filters.deadband(wiimote[n].nunchuck.stick.x, 5), -95, 95, -vJoy[n + 1].axisMax,
                                vJoy[n + 1].axisMax)
  vJoy[n].ry = -filters.mapRange(filters.deadband(wiimote[n].nunchuck.stick.y, 5), -95, 95, -vJoy[n + 1].axisMax,
                                 vJoy[n + 1].axisMax)
...
def map_nunchuck_buttons(n):
  ...
  NCRollDeg, _, _ = map_nunchuck_orientation(n)
  diagnostics.watch(NCRollDeg)

  vJoy[n].setButton(14, NCRollDeg > 15)
  vJoy[n].setButton(13, NCRollDeg < -15)
